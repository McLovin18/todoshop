...
import { obtenerCategoriasAlimentos, guardarCategoriaAlimento, eliminarCategoriaAlimento } from "../../lib/categorias-db";
...
export type CategoriaAlimento = {
  ...
  icono?: string;
  orden?: number;
  visibleEnNavbar?: boolean;
  subcategorias?: CategoriaAlimento[];
};

// Utilidad para agregar una categoría anidada en el path correcto
function addNestedCategoria(categorias: CategoriaAlimento[], path: number[], nueva: CategoriaAlimento): CategoriaAlimento[] {
...
function regenerateOrder(items: CategoriaAlimento[]): CategoriaAlimento[] {
...
function getByPath(categorias: CategoriaAlimento[], path: number[]): CategoriaAlimento[] | null {
...
function moveCategoria(categorias: CategoriaAlimento[], sourcePath: number[], destPath: number[], destLevel: number): CategoriaAlimento[] | null {
  // Validaciones
  ...
  const copy = JSON.parse(JSON.stringify(categorias)) as CategoriaAlimento[];
  ...
  let pointer: CategoriaAlimento[] = copy;
...
export default function CategoriasAlimentosAdminPanel({ onCategoriasChange }: { onCategoriasChange?: (cats: CategoriaAlimento[]) => void }) {
  ...
  useEffect(() => {
    obtenerCategoriasAlimentos().then(cats => setCategorias(cats || []));
  }, []);
  ...
  const agregarCategoria = async () => {
    ...
    const nueva: CategoriaAlimento = { 
      id: Date.now().toString(), 
      nombre: nuevaCategoria, 
      icono: nuevoIcono.trim() || undefined, 
      orden: categorias.length, 
      visibleEnNavbar: false,
      subcategorias: [] 
    };
    await guardarCategoriaAlimento(nueva);
    setCategorias(prev => [...prev, nueva]);
    setNuevaCategoria("");
    setNuevoIcono("");
  };

  const handleEliminarCategoria = async (cat: CategoriaAlimento, nivel: number, parentPath: number[] = []) => {
    if (nivel === 1) {
      await eliminarCategoriaAlimento(cat.id);
      ...
      setCategorias(prev => {
        ...
        guardarCategoriaAlimento(copy[parentPath[0]]);
        return copy;
      });
    }
  };
  ...
  const handleDrop = async (path: number[], level: number) => {
    ...
    if (newCategorias) {
      ...
      if (level === 1) {
        ...
        await Promise.all([
          guardarCategoriaAlimento(newCategorias[draggedPath.path[0]]),
          guardarCategoriaAlimento(newCategorias[path[0]])
        ]);
      } else {
        // Si reordena nivel 2 o superior, guardar la categoría raíz con toda la estructura
        await guardarCategoriaAlimento(newCategorias[path[0]]);
      }
    }
  ...
  const handleEditarNombre = async (path: number[], newName: string, level: number) => {
    ...
    setCategorias(prev => {
      const copy = JSON.parse(JSON.stringify(prev)) as CategoriaAlimento[];
      ...
      if (level === 1) {
        guardarCategoriaAlimento(copy[path[0]]);
      } else {
        guardarCategoriaAlimento(copy[path[0]]);
      }
  ...
  const handleToggleVisibleEnNavbar = async (path: number[], level: number) => {
    setCategorias(prev => {
      const copy = JSON.parse(JSON.stringify(prev)) as CategoriaAlimento[];

      if (level === 1) {
        copy[path[0]].visibleEnNavbar = !copy[path[0]].visibleEnNavbar;
        guardarCategoriaAlimento(copy[path[0]]);
      } else {
        let pointer = copy;
        for (let i = 0; i < path.length - 1; i++) {
        ...
        pointer[path[path.length - 1]].visibleEnNavbar = !pointer[path[path.length - 1]].visibleEnNavbar;
        guardarCategoriaAlimento(copy[path[0]]);
      }
  ...
  const renderCategorias = (cats: CategoriaAlimento[], nivel = 1, parentPath: number[] = []): JSX.Element => (
        ...
        return (
          <li key={cat.id} className="mb-1">
            <div
              ...
              } ${isDragOver ? 'bg-emerald-200 border-2 border-emerald-500' : 'bg-white'} ${
                !isEditing && nivel < 3 ? 'cursor-move hover:bg-gray-50' : ''
              }`}
              style={{
                borderRadius: '0.375rem',
                border: isDragOver ? '2px solid rgb(16, 185, 129)' : '1px solid rgb(229, 231, 235)',
              }}
            >
              {cat.subcategorias && cat.subcategorias.length > 0 && (
                <button
                  className="mr-1 text-emerald-700 focus:outline-none flex-shrink-0"
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <label className="flex items-center gap-1 cursor-pointer" title="Visible en navbar">
                    <input
                      type="checkbox"
                      checked={cat.visibleEnNavbar || false}
                      onChange={() => handleToggleVisibleEnNavbar(path, nivel)}
                      className="w-4 h-4 accent-emerald-600"
                    />
                    <span className="text-xs text-gray-600">Navbar</span>
                  </label>
                  <button
                  ...
                  {nivel < 3 && (
                    <button
                      className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded hover:bg-emerald-200 flex-shrink-0"
                      onClick={async () => {
                        ...
                        if (nombre && nombre.trim()) {
                          ...
                          await guardarCategoriaAlimento(newCat[path[0]]);
                        }
                      }}
                    >
                      + {nivel === 1 ? "Sub" : "SubSub"}
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow p-6">
      <h2 className="text-xl font-bold mb-2 text-emerald-700">Gestión de Categorías de Alimentos</h2>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <input
          ...
          placeholder="Nueva categoría de alimento..."
      ...
      <div className="border rounded-lg p-4 bg-gray-50">
        {categorias.length > 0 ? renderCategorias(categorias) : <p className="text-gray-500">No hay categorías de alimentos. Agrega una nueva.</p>}
      </div>
    </div>
  );
}
